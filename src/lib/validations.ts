...
export const createTaskSchema = z.object({
  ...
  description: z.string().trim().max(5000, { message: 'Description must be less than 5000 characters' }).optional(),
  dueDate: z.date().optional().nullable(),
  priority: z.enum(['low', 'medium', 'high', 'urgent'], { message: 'Invalid priority' }),
  status: z.enum(['todo', 'in_progress', 'review', 'completed'], { message: 'Invalid status' }),

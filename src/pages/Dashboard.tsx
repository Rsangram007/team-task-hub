...
export default function Dashboard() {
  ...
  const stats = [
    ...
    { label: 'Completed', value: allTasks.filter(t => t.status === 'completed').length, icon: CheckSquare },
  ];
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="icon" className="relative">
                  ...
                  {unreadCount > 0 && <Badge className="absolute -top-1 -right-1 h-5 w-5 p-0 flex items-center justify-center text-xs">{unreadCount}</Badge>}
                </Button>
              </PopoverTrigger>
                  ...
                  {notifications.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {notifications.slice(0, 10).map(n => (
                        <div key={n.id} className={`p-2 rounded-lg text-sm ${n.read ? 'opacity-60' : 'bg-muted'}`} onClick={() => !n.read && markRead.mutate(n.id)}>
                          <p className="font-medium">{n.title}</p>
                          <p className="text-muted-foreground text-xs">{n.message}</p>
                          <p className="text-muted-foreground text-xs mt-1">{format(new Date(n.created_at), 'MMM d, h:mm a')}</p>
                        </div>
                      ))}
                    </div>
                  )}
      ...
      <main className="container mx-auto px-4 py-8 relative z-10">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground">Manage your tasks and collaborate with your team</p>
          </div>
          <Button onClick={handleCreate} className="gap-2"><Plus className="h-4 w-4" />New Task</Button>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <TaskFiltersBar filters={filters} sort={sort} onFiltersChange={setFilters} onSortChange={setSort} />
          </div>
          <TabsContent value={activeTab} className="mt-4">
            <TaskList tasks={getDisplayedTasks()} isLoading={isLoading} onEdit={handleEdit} onDelete={handleDelete} />
          </TabsContent>
        </Tabs>
      </main>
